...
Drei Schritte:
  1. Nur die größte zusammenhängende Fläche behalten — Vision lässt manchmal
     Reste stehen (eine Schachtel im Hintergrund, ein Stativbein).
...
def groesste_flaeche(alpha: np.ndarray, schwelle: int = 24) -> np.ndarray:
    ...
    fest = alpha > schwelle
    h, w = fest.shape
    besucht = np.zeros_like(fest, dtype=bool)
...
def aufbereiten(pfad_ein: str, pfad_aus: str, breite: int, verhaeltnis=(4, 5)) -> None:
    ...
    kasten = im.getchannel('A').point(lambda p: 255 if p > 24 else 0).getbbox()
    if kasten:
        im = im.crop(kasten)
